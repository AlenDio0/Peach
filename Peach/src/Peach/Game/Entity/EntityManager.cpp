...
namespace Peach
{
	...
	UUID EntityManager::addEntity(Ref<Entity> entity)
	{
		UUID uuid;
	...
	UUID EntityManager::addEntity(Entity* entity)
	{
		return addEntity(Ref<Entity>(entity));
	}

	void EntityManager::removeEntity(UUID uuid)
	{
		PEACH_CORE_TRACE("EntityManager::removeEntity(uuid: {})", uuid);
		m_Entities.erase(uuid);
	}

	void EntityManager::removeEntity(Ref<Entity> entity)
	{
